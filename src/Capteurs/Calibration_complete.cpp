...
int main (int argc , char** argv)
{
 ...
 mpu.begin(ACCEL_RANGE_4G, GYRO_RANGE_500DPS);
 ...
 if ( i == 2 )
 {
  ...
  float somme_gx_mpu = 0 , somme_gy_mpu = 0 , somme_gz_mpu = 0;
 float somme_ax_mpu = 0 , somme_ay_mpu = 0 , somme_az_mpu = 0; 
  float moyenne_ax_mpu , moyenne_ay_mpu , moyenne_az_mpu;
  float moyenne_gx_mpu , moyenne_gy_mpu , moyenne_gz_mpu;
 
  int compte;
  int count = 0;
  for ( compte = 1; compte < 15000; compte++ )
  {
   usleep (10);
  ...
  moyenne_ax_mpu = somme_ax_mpu / count;
  moyenne_ay_mpu = somme_ay_mpu / count;
  moyenne_az_mpu = somme_az_mpu / count;

  moyenne_gx_mpu = somme_gx_mpu / count;
  moyenne_gy_mpu = somme_gy_mpu / count;
  moyenne_gz_mpu = somme_gz_mpu / count;

  cout << "gyrox_offset_MPU = " << moyenne_gx_mpu << endl;
  cout << "gyroy_offset_MPU = " << moyenne_gy_mpu << endl;
  cout << "gyroz_offset_MPU = " << moyenne_gz_mpu << endl;

  cout << "accelx_offset_MPU = " << moyenne_ax_mpu << endl;
  cout << "accely_offset_MPU = " << moyenne_ay_mpu << endl;
  cout << "accelz_offset_MPU = " << moyenne_az_mpu << endl;
 ...
 if ( i == 4 )
 {
  ...
  gpioSetPWMfrequency(moteur_devant_droit, 490);
  gpioSetPWMfrequency(moteur_devant_gauche, 490);
  gpioSetPWMfrequency(moteur_deriere_gauche, 490);
  gpioSetPWMfrequency(moteur_deriere_droit,490);
  ...
  roulis = temps_haut_roulis;
   gpioPWM(moteur_devant_gauche, gaz);
   gpioPWM(moteur_deriere_gauche, gaz);
   gpioPWM(moteur_deriere_droit, gaz);
   gpioPWM(moteur_devant_droit, gaz);
   //roulis = -((float)temps_haut_roulis- 1500.0)/10.0;
   //roulis = (fabs(roulis)*roulis)/40;
   //printf(" gaz = %d\n",gaz);
   //printf(" roulis = %d\n",roulis);
   usleep(attente);
...
int do_accel_calibration_measurements (double accel_offs_MPU[3] , double accel_T_MPU[3][3] , MPU9250 * mpu )
{
 const int samples_num = 5000;	//RF_ONERA
...
void recuperation_cmd_gaz(int gpio, int level, uint32_t tick)
{
 ...
 else
 {
  ...
  //printf("GPIO temps_haut_gaz precedent = %d \n", tick_precedent_gaz);
